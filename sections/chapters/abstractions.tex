...
\todo{mkb: verstehe hier die Notation nicht ganz. A ist ein Mapping von Termen auf Formeln, und bei drei Beispielen gibt es drei Notationen, einmal mit Kommata getrennt, dann als Folgerung und dann mit einer Gleichetzung. Ersetzt das Mapping A, jegliches Mapping von der ursprünglichen Multiplikation oder wird davon ausgegangen dass das Mapping gleichbleibt außer für nochmal explizit definierte Terme?}
...
This is rather impractical when we only event \todo{mkb: event?} want to abstract the first $w$ steps of multiplication.
...
\todo{mkb: for a operation op(a,b), sonst sind a und b hier nicht festgelegt}
...
\todo{mkb: finde Fußnoten in Formeln sehr unschön, da es auch ein Index oder sonsitiges sein könnte. Lieber wie bei udiv einmal im Satz danach benennen.}
