...
In this chapter\todo{mkb: ,} we will introduce the foundations of Boolean algebra and SAT solving. Based on these techniques we will introduce SMT solving and present two predominant SMT solving approaches.

\section{Basic boolean algebra and notation}
The structure and notation of this section is\todo{mkb: are based on} based on \cite{Marques-Silva-PropositionalSATSolving} and \cite{fondements-logique}.
...
\end{definition}
We define $var(\mathcal{F})$ as the set of all variables appearing in  $\mathcal{F}$.\\\todo{mkb: Umbruch weglassen}
...
\end{definition}
For terms $t$, variables $v$ or predicates $p$ we denote their value according to a $\Sigma$-interpretation $\mathcal{I}$ as $t^\mathcal{I}$, $v^\mathcal{I}$ and $p^\mathcal{I}$\todo{mkb: .}
...
\end{definition}
\todo{mkb: finde es als enumeration oder itemize einfacher zu lesen}
...
\todo{mkb: Beschreiben was \bar{x} und \bar{y} sind, bitvectors or variables?} This theory can be axiomatized throught the \textit{function congruence axiom} \cite{PreinerNiemetzBiere-DIFTS13}:
\begin{equation}
    \tag{EUF}
...
\todo{mkb: Generell: "blablalba, we" vor wie fast immer ein Komma.}
